...
type CarQuery struct {
	...
	// eager-loading edges.
	withCarPredicament *PredicamentQuery
...
// QueryCarPredicament chains the current query on the car_predicament edge.
func (cq *CarQuery) QueryCarPredicament() *PredicamentQuery {
	query := &PredicamentQuery{config: cq.config}
...
//  WithCarPredicament tells the query-builder to eager-loads the nodes that are connected to
// the "car_predicament" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CarQuery) WithCarPredicament(opts ...func(*PredicamentQuery)) *CarQuery {
	query := &PredicamentQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCarPredicament = query
	return cq
}
...
func (cq *CarQuery) sqlAll(ctx context.Context) ([]*Car, error) {
	...
	if query := cq.withCarPredicament; query != nil {
		...
		query.Where(predicate.Predicament(func(s *sql.Selector) {
			s.Where(sql.InValues(car.CarPredicamentColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.carid
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "carid" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "carid" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.CarPredicament = append(node.Edges.CarPredicament, n)
		}
	}
