...
type UserQuery struct {
	...
	// eager-loading edges.
	withUserPredicament *PredicamentQuery
...
// QueryUserPredicament chains the current query on the user_predicament edge.
func (uq *UserQuery) QueryUserPredicament() *PredicamentQuery {
	query := &PredicamentQuery{config: uq.config}
...
//  WithUserPredicament tells the query-builder to eager-loads the nodes that are connected to
// the "user_predicament" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithUserPredicament(opts ...func(*PredicamentQuery)) *UserQuery {
	query := &PredicamentQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserPredicament = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withUserPredicament; query != nil {
		...
		query.Where(predicate.Predicament(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserPredicamentColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.UserPredicament = append(node.Edges.UserPredicament, n)
		}
	}
